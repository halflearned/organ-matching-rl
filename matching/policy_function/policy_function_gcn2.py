...
if __name__ == "__main__":
    ...
    for i in range(int(1e8)):
        ...
        loss, out = net.run(A[b_idx], Z[b_idx], Y[b_idx])
        if i % log_every == 0:
            ls = X.any(2).sum(1)
            y_true = np.concatenate([Y[b_idx[k],:ls[k]] \
                                     for k in range(batch_size)])
            y_hat = np.concatenate([out[k,:ls[k],1:] > out[k,:ls[k],:1] \
                                    for k in range(batch_size)])
            loss = loss.round(4)
            tp = np.mean(y_true & y_hat)
            fp = np.mean(~y_true & y_hat)
            tn = np.mean(~y_true & ~y_hat)
            fn = np.mean(y_true & ~y_hat)
            
            msg = "{:1.4f},{:1.4f},{:1.4f},{:1.4f},{:1.4f}"\
            .format(loss,tp,tn,fp,fn)
            print(msg)
            print(msg,
                  file = open("results/" + str(net) + ".txt", "a"))
